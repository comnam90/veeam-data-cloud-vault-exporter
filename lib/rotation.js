...
export async function rotateOne(vault, regenerateKeyUrl, deps = {}) {
  ...
  try {
    ...
    // Read body as text first to distinguish empty body from non-JSON body.
    let bodyText = '';
    try { bodyText = (await response.text()).trim(); } catch (_) { /* ignore */ }
    if (!bodyText) {
    ...
    try { data = JSON.parse(bodyText); } catch (_) {
      return { vault, response: null, error: `API returned non-JSON response: ${bodyText.slice(0, 100)}`, anomaly: null, timestamp };
    }
