...
export async function rotateOne(vault, regenerateKeyUrl, deps = {}) {
  ...
  try {
    ...
    if (!data) {
      // Rotation succeeded but API returned no body — keys rotated but not returned.
      return { vault, response: null, error: null, anomaly: 'Key rotated but API returned no credentials', timestamp };
    }
